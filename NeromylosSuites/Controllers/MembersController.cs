...
namespace NeromylosSuites.Controllers
{
    ...
    public class MembersController : ControllerBase
    {
        ...
        [HttpGet]
        [Authorize(Roles = "ADMIN,RECEPTIONIST")]
        [ProducesResponseType(typeof(PaginatedResult<MemberReadOnlyDTO>), StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status401Unauthorized)]
        [ProducesResponseType(StatusCodes.Status403Forbidden)]
        public async Task<ActionResult<PaginatedResult<MemberReadOnlyDTO>>> GetPaginatedMembers(
            [FromQuery] int pageNumber = 1,
            [FromQuery] int pageSize = 10)
        {
            var result = await _applicationService.MemberService
                .GetPaginatedMembersAsync(pageNumber, pageSize);
        ...
        {
            var result = await _applicationService.MemberService
                .GetPaginatedMembersFilteredAsync(pageNumber, pageSize, filters ?? new UserFiltersDTO());
